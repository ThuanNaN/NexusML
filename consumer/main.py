...
class ImageProcessor:
    ...
    def preprocess_image(self, image_data_str):
        ...
        try:
            ...
            if img.format not in ['JPEG', 'PNG', 'WEBP', 'GIF', None]:
    ...
    def save_to_database(self, metadata, grayscale_img, original_img):
        ...
        try:
            ...
            # Add image shape information to metadata
            metadata['processed_shape'] = {
                'original': [TARGET_SIZE[1], TARGET_SIZE[0], 3],
                'grayscale': [TARGET_SIZE[1], TARGET_SIZE[0], 1]
            }

            # Insert data into the database
            cur.execute("""
                INSERT INTO images (image_id, image_url, size, format, grayscale_data, original_data, processed_shape)
                VALUES (%s, %s, %s, %s, %s, %s, %s)
                ON CONFLICT (image_id) DO NOTHING
            """, (
                metadata['image_id'],
                metadata.get('image_url', metadata['image_id']),
                json.dumps(metadata.get('size', {'width': TARGET_SIZE[0], 'height': TARGET_SIZE[1]})),
                metadata.get('format', 'unknown'),
                grayscale_data,
                original_data,
                json.dumps(metadata['processed_shape'])
            ))
...
class KafkaImageConsumer:
    def __init__(self):
        ...
        self.consumer = KafkaConsumer(
            ...
            max_poll_records=BATCH_SIZE,  # Process messages in batches
        ...
        try:
            ...
            cur.execute("""
                CREATE TABLE IF NOT EXISTS images (
                    ...
                    size JSONB,
                    format VARCHAR(50),
                    grayscale_data BYTEA,
                    original_data BYTEA,
                    processed_shape JSONB,
            ...
            cur.execute("""
                CREATE INDEX IF NOT EXISTS idx_image_id ON images (image_id)
            """)
